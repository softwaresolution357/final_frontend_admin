...
exports.postHomePage = async (req, res, next) => {
  ...
  if (count == 0) {
    try {
      ...
      if (parallex != '') {
        data.parallex = parallex;
      }
      if (parallexdescription != '') {
        data.parallexdescription = parallexdescription;
      }
...
exports.updateHomePage = async (req, res, next) => {
  ...
  if (count == 1) {
    try {
      ...
      if(parallex != undefined){
        data.parallex = parallex;
      }else{
        await home.findByIdAndUpdate({ _id: req.params.id }, {$unset:{parallex:1}}, { new: true });
      }
      
      if(parallexdescription != undefined){
        data.parallexdescription = parallexdescription;
      }else{
        await home.findByIdAndUpdate({ _id: req.params.id }, {$unset:{parallexdescription:1}}, { new: true });
      }
